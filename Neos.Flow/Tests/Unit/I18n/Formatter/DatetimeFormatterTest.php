...
class DatetimeFormatterTest extends UnitTestCase
{
    ...
    public function formatMethodsAreChoosenCorrectly()
    {
        ...
        $formatter->expects(self::once())->method('formatDateTime')->with($this->sampleDateTime, $this->sampleLocale, I18n\Cldr\Reader\DatesReader::FORMAT_LENGTH_DEFAULT)->will(self::returnValue('bar1'));
        $formatter->expects(self::once())->method('formatDate')->with($this->sampleDateTime, $this->sampleLocale, I18n\Cldr\Reader\DatesReader::FORMAT_LENGTH_DEFAULT)->will(self::returnValue('bar2'));
        $formatter->expects(self::once())->method('formatTime')->with($this->sampleDateTime, $this->sampleLocale, I18n\Cldr\Reader\DatesReader::FORMAT_LENGTH_FULL)->will(self::returnValue('bar3'));
