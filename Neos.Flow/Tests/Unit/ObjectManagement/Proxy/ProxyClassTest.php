...
class ProxyClassTest extends UnitTestCase
{
    ...
    public function renderWorksAsExpected($originalClassName, $originalClassAnnotations, $originalClassDocumentation, $originalClassConstants, $expectedProxyCode)
    {
        ...
        $mockReflectionService->expects($this->any())->method('isClassAbstract')->will(self::returnValue(strpos($expectedProxyCode, 'abstract ') !== false));
        $mockReflectionService->expects($this->any())->method('getClassAnnotations')->will(self::returnValue($originalClassAnnotations));

        $mockProxyClass = $this->getAccessibleMock(ProxyClass::class, ['buildClassDocumentation'], [$originalClassName], '', true);
        $mockProxyClass->expects($this->any())->method('buildClassDocumentation')->will(self::returnValue($originalClassDocumentation));
