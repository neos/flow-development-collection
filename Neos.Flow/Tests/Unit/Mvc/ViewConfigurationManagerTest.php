...
class ViewConfigurationManagerTest extends \Neos\Flow\Tests\UnitTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->mockCache->expects($this->any())->method('get')->will(self::returnValue(false));
        $this->inject($this->viewConfigurationManager, 'cache', $this->mockCache);

        // a dummy request is prepared
        $this->mockActionRequest = $this->getMockBuilder(ActionRequest::class)->disableOriginalConstructor()->getMock();
        $this->mockActionRequest->expects($this->any())->method('getControllerPackageKey')->will(self::returnValue('Neos.Flow'));
        $this->mockActionRequest->expects($this->any())->method('getControllerSubpackageKey')->will(self::returnValue(''));
        $this->mockActionRequest->expects($this->any())->method('getControllerName')->will(self::returnValue('Standard'));
        $this->mockActionRequest->expects($this->any())->method('getControllerActionName')->will(self::returnValue('index'));
        $this->mockActionRequest->expects($this->any())->method('getFormat')->will(self::returnValue('html'));
        $this->mockActionRequest->expects($this->any())->method('getParentRequest')->will(self::returnValue(null));
    }
    ...
    public function getViewConfigurationFindsMatchingConfigurationForRequest()
    {
        ...
        $this->mockConfigurationManager->expects($this->any())->method('getConfiguration')->with('Views')->will(self::returnValue($viewConfigurations));
