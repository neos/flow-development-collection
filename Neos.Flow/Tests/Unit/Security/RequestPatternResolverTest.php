...
class RequestPatternResolverTest extends UnitTestCase
{
    ...
    public function resolveRequestPatternClassThrowsAnExceptionIfNoRequestPatternIsAvailable()
    {
        ...
        $mockObjectManager->expects($this->any())->method('getClassNameByObjectName')->will(self::returnValue(false));
    ...
    {
        $mockObjectManager = $this->getMockBuilder(ObjectManager::class)->disableOriginalConstructor()->getMock();
        $mockObjectManager->expects($this->any())->method('getClassNameByObjectName')->with('ExistingRequestPatternClass')->will(self::returnValue('ExistingRequestPatternClass'));
