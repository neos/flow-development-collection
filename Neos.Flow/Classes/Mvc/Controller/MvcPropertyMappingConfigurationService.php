...
class MvcPropertyMappingConfigurationService
{
        ...
        foreach ($formFieldNames as $formField) {
            ...
            for ($i = 0; $i < $formFieldPartsCount; $i++) {
                ...
                if (!is_array($currentPosition)) {
                    throw new InvalidArgumentForHashGenerationException('The form field "' . $formField . '" is declared as array, but it collides with a previous form field of the same name which declared the field as string. This is an inconsistency you need to fix inside your generated form. (String overridden by Array)', 1255072196);
                }
    ...
    public function initializePropertyMappingConfigurationFromRequest(ActionRequest $request, Arguments $controllerArguments)
    {
        ...
        foreach ($trustedProperties as $propertyName => $propertyConfiguration) {
            $argument = $controllerArguments->hasArgument($propertyName) ? $controllerArguments->getArgument($propertyName) : null;
            if ($argument === null) {
                continue;
            }
