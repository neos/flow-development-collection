 ...
 *
 */
class AppendConfigurationType extends AbstractConfigurationType
{
    /**
    ...
    public function process(YamlSource $configurationSource, string $configurationType, array $packages, array $configuration) : array
    {
        ...
        foreach ($packages as $package) {
            $configuration = array_merge($configuration, $configurationSource->load($package->getConfigurationPath() . $configurationType, $allowSplitSource));
        }
        ...
        foreach ($this->orderedListOfContextNames as $contextName) {
            foreach ($packages as $package) {
                $configuration = array_merge($configuration, $configurationSource->load($package->getConfigurationPath() . $contextName . '/' . $configurationType, $allowSplitSource));
            }
