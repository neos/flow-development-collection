...
abstract class MediaTypeHelper
{
     ...
     *
     * @param array<string> $acceptedMediaTypes A list of accepted media types according to a request.
     * @param array<string> $supportedMediaTypes A list of media types which are supported by the application / controller
     ...
     * @return array<mixed> The parsed list of field values, ordered by user preference
     */
    public static function parseContentNegotiationQualityValues(string $rawValues): array
    {
        $acceptedTypes = array_map(
            function ($acceptType) {
                ...
                if ($typeAndQuality === false) {
                    throw new \InvalidArgumentException('Invalid accept type ' . $acceptType, 1744449962);
                }
