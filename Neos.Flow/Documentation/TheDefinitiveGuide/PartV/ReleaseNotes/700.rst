...
See `HTTP ComponentChain and ComponentContext`_ for upgrade instructions and :ref:`ch-routing` for comprehensive documentation.
...
output::
  ...
  +----+---------------------------------------------------------------------------+----------------+------------------------------------------------------------------------------------------+
  | 1  | neos/media/browser/images(/{@action}).{@format}                           | any            | Neos.Media.Browser :: Image Browser                                                      |
  | 2  | neos/media/browser/assets(/{@action}).{@format}                           | any            | Neos.Media.Browser :: Asset Browser                                                      |
  | 3  | neos/media/browser/asset-proxies(/{@action}).{@format}                    | any            | Neos.Media.Browser :: Asset Proxy Service                                                |
  | 4  | neos/media/browser/image-variant(/{@action}).{@format}                    | any            | Neos.Media.Browser :: ImageVariant Service                                               |
  | 5  | media/thumbnail/{thumbnail}                                               | GET            | Neos.Media :: Thumbnail                                                                  |
  | .. | ...                                                                       | ...            | ...                                                                                      |
  | 78 | {node}robots.txt                                                          | any            | Neos.Seo :: robots.txt                                                                   |
  +----+---------------------------------------------------------------------------+----------------+------------------------------------------------------------------------------------------+

  Run ./flow routing:show <index> to show details for a route

See `Access Route Parameters when resolving routes`_ for upgrade instructions.
...
https://github.com/neos/flow-development-collection/pull/2166
...
Since Doctrines `OneToMany` annotation is always bidrectional and also dictates the owning side of the relation (at the unexpected side from a modelling PoV), it is not straightforward to model this correctly.
...
https://github.com/neos/flow-development-collection/pull/2056
