...
.. note::
  ...
  somewhere in Flow's directory structure – you should only deal with resource objects.
...
.. tip::
	...
	There are more API functions in Flow's ``ResourceManager`` which allow for retrieving
...
.. admonition:: Why Flow requires your OS to support symbolic links

  Publishing resources basically means copying files from a private location to the public
  web directory. Flow instead creates symbolic links, making the resource publishing
...
Persistent resources are published on demand because Flow cannot know which resources are
...
file names. That is why Flow adds a "virtual" file name to the resource, like this:
``http://example.local/_Resources/Persistent/107bed85ba5e9bae0edbae879bbc2c26d72033ab/my-speaking-title.jpg``.
Flow ships with a *mod_rewrite* rule to map the speaking titles to the hash files.
