<?php
namespace TYPO3\Flow\ObjectManagement\DependencyInjection;
...
use TYPO3\Flow\ObjectManagement\Exception as ObjectException;
use TYPO3\Flow\ObjectManagement\ObjectManagerInterface;
use TYPO3\Flow\ObjectManagement\Proxy\Compiler;
use TYPO3\Flow\ObjectManagement\Proxy\ObjectSerializationTrait;
...
class ProxyClassBuilder
{
    ...
    protected function buildConstructorInjectionCode(Configuration $objectConfiguration)
    {
        ...
        foreach ($constructorParameterInfo as $parameterName => $parameterInfo) {
            ...
            if ($objectConfiguration->getScope() === Configuration::SCOPE_SINGLETON) {
                $code .= '        if (!array_key_exists(' . $index . ', $arguments)) throw new \TYPO3\Flow\ObjectManagement\Exception\UnresolvedDependenciesException(\'Missing required constructor argument $' . $parameterName . ' in class \' . __CLASS__ . \'. Please check your calling code and Dependency Injection configuration.\', 1296143787);' . "\n";
            } else {
                $code .= '        if (!array_key_exists(' . $index . ', $arguments)) throw new \TYPO3\Flow\ObjectManagement\Exception\UnresolvedDependenciesException(\'Missing required constructor argument $' . $parameterName . ' in class \' . __CLASS__ . \'. Note that constructor injection is only support for objects of scope singleton (and this is not a singleton) – for other scopes you must pass each required argument to the constructor yourself.\', 1296143788);' . "\n";
            }
            $index++;
        }
     ...
     *
     * @param Configuration $objectConfiguration
     * @param integer $cause a \TYPO3\Flow\ObjectManagement\ObjectManagerInterface::INITIALIZATIONCAUSE_* constant which is the cause of the initialization command being called.
     * @return string
     */
