...
class DateTimeConverter extends AbstractTypeConverter
{
     ...
     * @return \DateTime
     * @throws PropertyException\TypeConverterException
     */
    public function convertFrom($source, $targetType, array $convertedChildProperties = [], PropertyMappingConfigurationInterface $configuration = null)
    {
        ...
        if (is_string($source)) {
            ...
            if (isset($source['date']) && is_string($source['date'])) {
                ...
                if ($source['day'] < 1 || $source['month'] < 1 || $source['year'] < 1) {
                    return new ValidationError('Could not convert the given date parts into a DateTime object because one or more parts were 0.', 1333032779);
                }
                $dateAsString = sprintf('%d-%d-%d', $source['year'], $source['month'], $source['day']);
            } else {
                throw new PropertyException\TypeConverterException('Could not convert the given source into a DateTime object because it was not an array with a valid date as a string', 1308003914);
            }
        ...
        if (is_array($source) && isset($source['timezone']) && strlen($source['timezone']) !== 0) {
            try {
                ...
                throw new PropertyException\TypeConverterException('The specified timezone "' . $source['timezone'] . '" is invalid.', 1308240974);
            }
        ...
        if ($date === false) {
            return new ValidationError('The date "%s" was not recognized (for format "%s").', 1307719788, [$dateAsString, $dateFormat]);
        }
        if (is_array($source)) {
     ...
     * @param PropertyMappingConfigurationInterface $configuration
     * @return string
     * @throws PropertyException\InvalidPropertyMappingConfigurationException
     */
    protected function getDefaultDateFormat(PropertyMappingConfigurationInterface $configuration = null)
    {
        ...
        if ($dateFormat === null) {
            ...
            throw new PropertyException\InvalidPropertyMappingConfigurationException('CONFIGURATION_DATE_FORMAT must be of type string, "' . (is_object($dateFormat) ? get_class($dateFormat) : gettype($dateFormat)) . '" given', 1307719569);
        }
        return $dateFormat;
    }
