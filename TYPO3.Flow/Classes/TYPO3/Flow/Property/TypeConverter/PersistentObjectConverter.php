...
class PersistentObjectConverter extends ObjectConverter
{
    ...
    {
        $configuredTargetType = $configuration->getConfigurationFor($propertyName)->getConfigurationValue(self::class, self::CONFIGURATION_TARGET_TYPE);
    ...
    {
        if (is_array($source)) {
            if ($this->reflectionService->isClassAnnotatedWith($targetType, Flow\ValueObject::class)) {
    ...
    {
        if (!isset($source['__identity'])) {
            if ($this->reflectionService->isClassAnnotatedWith($targetType, Flow\ValueObject::class) === true) {
                // Allow creation for ValueObjects by default, but prevent if explicitly disallowed
                if ($configuration !== null && $configuration->getConfigurationValue(self::class, self::CONFIGURATION_CREATION_ALLOWED) === false) {
            ...
            } elseif ($configuration === null || $configuration->getConfigurationValue(self::class, self::CONFIGURATION_CREATION_ALLOWED) !== true) {
        ...
        } elseif ($configuration !== null && $configuration->getConfigurationValue(self::class, self::CONFIGURATION_IDENTITY_CREATION_ALLOWED) === true) {
            ...
            if (count($convertedChildProperties) > 0 && ($configuration === null || $configuration->getConfigurationValue(self::class, self::CONFIGURATION_MODIFICATION_ALLOWED) !== true)) {
