...
class F3_FLOW3_MVC_RequestProcessorChainManager {
	...
	public function unregisterRequestProcessor(F3_FLOW3_MVC_RequestProcessorInterface $requestProcessor, $requestType = NULL) {
		if ($requestType !== NULL) {
			if (!in_array($requestType, $this->supportedRequestTypes, TRUE)) throw new F3_FLOW3_MVC_Exception_InvalidRequestType('"' . $requestType . '" is not a valid request type - or at least it\'s not supported by the Request Processor Chain.', 1187261072);
