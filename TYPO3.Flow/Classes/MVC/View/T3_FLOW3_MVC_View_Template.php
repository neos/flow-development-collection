 ...
 * @scope		prototype
 */
class T3_FLOW3_MVC_View_Template extends T3_FLOW3_MVC_View_Abstract {
	 ...
	 * @return void
	 * @throws T3_FLOW3_MVC_Exception_InvalidMarker if the marker is not a valid string
	 * @author Robert Lemke <robert@typo3.org>
	 */
	public function setMarkerContent($marker, $content) {
		if (!is_string($marker)) throw new T3_FLOW3_MVC_Exception_InvalidMarker('A template marker must be a valid string, ' . gettype($marker) . ' given.', 1187334295);
	 ...
	 * @return void
	 * @throws T3_FLOW3_MVC_Exception_InvalidPart if the part marker is not a valid string
	 * @author Robert Lemke <robert@typo3.org>
	 */
	public function setPartContent($partMarker, $content) {
		if (!is_string($partMarker)) throw new T3_FLOW3_MVC_Exception_InvalidPart('A template part marker must be a valid string, ' . gettype($partMarker) . ' given.', 1187334296);
	 ...
	 * @throws T3_FLOW3_MVC_Exception_InvalidTemplateResource if no template resource has been defined yet
	 * @author Robert Lemke <robert@typo3.org>
	 */
	public function render() {
		if ($this->templateResource == '') throw new T3_FLOW3_MVC_Exception_InvalidTemplateResource('No template resource has been defined yet.', 1187860750);
		$output = $this->templateResource;
		foreach ($this->markers as $marker => $content) {
			$output = str_replace('###' . T3_PHP6_Functions::strtoupper($marker) . '###', $content, $output);
		}

		foreach($this->parts as $marker => $content) {
			$output = preg_replace('/<!--\s*###' . T3_PHP6_Functions::strtoupper(preg_quote($marker, '/')) . '###.*###' . T3_PHP6_Functions::strtoupper(preg_quote($marker, '/')) . '###.*-->/msU', $content, $output);
		}
		return $output;
	}
