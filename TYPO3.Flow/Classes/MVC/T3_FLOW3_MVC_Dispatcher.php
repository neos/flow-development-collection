...
class T3_FLOW3_MVC_Dispatcher {
	...
	public function dispatch(T3_FLOW3_MVC_Request $request, T3_FLOW3_MVC_Response $response) {
		...
		if ($this->componentManager->isComponentRegistered($controllerName)) {	
			$controller = $this->componentManager->getComponent($controllerName);
			if (!$controller instanceof T3_FLOW3_MVC_Controller_RequestHandlingController) {
				throw new T3_FLOW3_MVC_Exception('Invalid controller "' . $controllerName . '". The controller must be a valid request handling controller.');
			}
