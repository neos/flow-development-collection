...
class T3_FLOW3_MVC_Controller_ActionController extends T3_FLOW3_MVC_Controller_RequestHandlingController {
	...
	protected function initializeView() {
		...
		if (is_array($explodedControllerName)) {
			$explodedRequestType = explode('_', get_class($this->request));
			if ($explodedRequestType[2] == 'MVC' && $explodedRequestType[4] == 'Request') {
				$possibleSpecializedViewName = 'T3_' . $explodedControllerName[1] . '_View_' . $explodedRequestType[3] . '_' . T3_PHP6_Functions::ucfirst($this->request->getActionName());
				if ($this->componentManager->isComponentRegistered($possibleSpecializedViewName)) {
					$this->view = $this->componentManager->getComponent($possibleSpecializedViewName);
					return;
				}
			}
			$possibleViewName = 'T3_' . $explodedControllerName[1] . '_View_' . T3_PHP6_Functions::ucfirst($this->request->getActionName());
