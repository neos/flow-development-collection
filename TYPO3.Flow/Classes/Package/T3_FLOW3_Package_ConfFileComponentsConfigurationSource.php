...
class T3_FLOW3_Package_ConfFileComponentsConfigurationSource implements T3_FLOW3_Package_ComponentsConfigurationSourceInterface {
	 ...
	 * Constructor
	 *
	 * @param  T3_FLOW3_AOP_Framework $aopFramework: A reference to the AOP framework
	 * @return void
	...
	protected function transformConfigurationArrayToObject($componentName, $componentConfigurationArray) {
		...
		foreach ($componentConfigurationArray as $optionName => $optionValue) {
			...
			switch ($optionNameParts[0]) {
				...
				break;
				case 'properties':
					$propertyType = (isset($optionNameParts[2]) && $optionNameParts[2] == 'reference') ? T3_FLOW3_Component_ConfigurationProperty::PROPERTY_TYPES_REFERENCE : T3_FLOW3_Component_ConfigurationProperty::PROPERTY_TYPES_STRAIGHTVALUE;
					$property = new T3_FLOW3_Component_ConfigurationProperty($optionNameParts[1], $optionValue, $propertyType);
					$parsedComponentConfiguration->setProperty($property);
				break;
				case 'constructorArguments':
					if (intval($optionNameParts[1]) != $optionNameParts[1]) throw new T3_FLOW3_Package_Exception_InvalidComponentConfiguration('Index of constructor arguments must be of type integer', 1168006024);
					$constructorArgumentType = (isset($optionNameParts[2]) && $optionNameParts[2] == 'reference') ? T3_FLOW3_Component_ConfigurationArgument::ARGUMENT_TYPES_REFERENCE : T3_FLOW3_Component_ConfigurationArgument::ARGUMENT_TYPES_STRAIGHTVALUE;
					$constructorArgument = new T3_FLOW3_Component_ConfigurationArgument(intval($optionNameParts[1]), $optionValue, $constructorArgumentType);
	...
	protected function parseScope($value) {
		if (!in_array($value, array('singleton', 'prototype', 'session'))) throw new T3_FLOW3_Package_Exception_InvalidComponentConfiguration('Invalid scope', 1167574991);
		return $value;
	}
