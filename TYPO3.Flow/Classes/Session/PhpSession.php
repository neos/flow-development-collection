...
class PhpSession implements \TYPO3\FLOW3\Session\SessionInterface {
	...
	public function injectSettings(array $settings) {
		...
		if (array_key_exists('PHPSession', $this->settings['session'])) {
			$this->settings['session']['PhpSession'] = array_merge($this->settings['session']['PhpSession'], $this->settings['session']['PHPSession']);
			unset($this->settings['session']['PHPSession']);
		}
	}
