 ...
 */
class T3_FLOW3_Error_ExceptionHandler {

	/**
	 * Constructs this exception handler
	 * 
	...
	protected function echoExceptionWeb(Exception $exception) {
		$pathPosition = strpos($exception->getFile(), TYPO3_PATH_ROOT);		
	...
	protected function getBacktraceCode(array $trace) {
		...
		if (count($trace)) {
			foreach ($trace as $index => $step) {
				if (isset($step['file'])) {
					$pathPosition = strpos($step['file'], TYPO3_PATH_ROOT);		
					$stepFileName = ($pathPosition === 0) ? substr($step['file'], strlen(TYPO3_PATH_ROOT)) : $step['file'];
				} else {
				...
				if (isset($step['args']) && is_array($step['args'])) {
					foreach ($step['args'] as $argument) {
						...
						if (is_object($argument)) {
							...
							$preparedArgument = (strlen($argument) < 40) ? $argument : substr($argument, 0, 40) . '…';
