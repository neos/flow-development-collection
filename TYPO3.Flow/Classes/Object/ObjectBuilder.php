...
/**
 * The Object Object ObjectBuilder takes care of the whole building (instantiation) process of an
...
class ObjectBuilder {
	...
	public function createObject($objectName, \F3\FLOW3\Object\Configuration\Configuration $objectConfiguration, array $overridingArguments = array()) {
		if (isset ($this->objectsBeingBuilt[$objectName])) throw new \F3\FLOW3\Object\Exception\CannotBuildObjectException('Circular object dependency for object "' . $objectName . '".', 1168505928);
		try {
			...
			if (class_exists($className) === FALSE && $customFactoryClassName === NULL) throw new \F3\FLOW3\Object\Exception\UnknownClassException('Class "' . $className . '" which was specified in the object configuration of object "' . $objectName . '" does not exist.', 1229967561);
				...
				switch ($argument->getType()) {
					...
					case \F3\FLOW3\Object\Configuration\ConfigurationArgument::ARGUMENT_TYPES_SETTING:
						if (strpos($argumentValue, '.') !== FALSE) {
							...
							$value = $this->configurationManager->getConfiguration(\F3\FLOW3\Configuration\ConfigurationManager::CONFIGURATION_TYPE_SETTINGS, $argumentValue);
						}
