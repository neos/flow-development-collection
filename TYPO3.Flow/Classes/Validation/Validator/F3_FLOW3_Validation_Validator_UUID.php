 ...
 */
class UUID implements F3::FLOW3::Validation::ValidatorInterface {
	...
	public function isValidProperty($propertyValue, F3::FLOW3::Validation::Errors &$errors) {
		if (!preg_match('/([a-f0-9]){8}-([a-f0-9]){4}-([a-f0-9]){4}-([a-f0-9]){4}-([a-f0-9]){12}/', $propertyValue)) {
			$errors->append($this->createNewValidationErrorObject('The given subject was not a valid UUID. Got: "' . $propertyValue . '"', 1221565853));
			return FALSE;
		}
