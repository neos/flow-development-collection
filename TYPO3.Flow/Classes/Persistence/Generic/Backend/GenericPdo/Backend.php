...
class Backend extends \TYPO3\FLOW3\Persistence\Generic\Backend\AbstractSqlBackend {
	...
	protected function buildPropertiesArray(\PDOStatement $propertyStatement, $className) {
		...
		foreach ($propertyStatement->fetchAll(\PDO::FETCH_ASSOC) as $propertyRow) {
			...
			if (!$this->reflectionService->getClassSchema($className)->hasProperty($propertyRow['name'])) continue;

				// we have a value (including NULL) on shelf
			if (isset($propertyRow['type'])) {
				$propertyMetadata = $this->reflectionService->getClassSchema($className)->getProperty($propertyRow['name']);
	...
	protected function getPlainValue($input) {
		...
		} elseif (is_object($input) && $this->persistenceSession->getIdentifierByObject($input) !== NULL) {
			return $this->persistenceSession->getIdentifierByObject($input);
		} else {
			return $input;
		}
	}
