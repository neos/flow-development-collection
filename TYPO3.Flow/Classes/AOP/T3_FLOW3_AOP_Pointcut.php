...
class T3_FLOW3_AOP_Pointcut implements T3_FLOW3_AOP_PointcutInterface {
	...
	/**
	 * @var T3_FLOW3_AOP_PointcutFilterComposite: The filter composite object, created from the pointcut expression
	 */
	 ...
	 * @param  T3_FLOW3_AOP_PointcutExpressionParserInterface $pointcutExpressionParser: The parser to use for parsing the pointcut expression
	...
	public function __construct($pointcutExpression, T3_FLOW3_AOP_PointcutExpressionParser $pointcutExpressionParser, $aspectClassName, $pointcutMethodName = NULL) {
		if (!is_string($pointcutExpression) || T3_PHP6_Functions::strlen($pointcutExpression) == 0) throw new T3_FLOW3_AOP_Exception_InvalidPointcutExpression('Pointcut expression must be a valid string, ' . gettype($pointcutExpression) . ' given.', 1202902188);
