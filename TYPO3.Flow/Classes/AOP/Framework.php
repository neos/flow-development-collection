	...
	protected function getProxyableClasses(array $classNames) {
		...
		foreach ($classNames as $className) {
			if (!in_array(substr($className, 0, 12), $this->blacklistedSubPackages)) {
				if (!$this->reflectionService->isClassReflected($className)) throw new \F3\FLOW3\AOP\Exception\UnknownClassException('The class "' . $className . '" does not exist.', 1187348208);
