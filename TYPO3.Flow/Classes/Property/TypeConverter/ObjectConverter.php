...
class ObjectConverter extends \TYPO3\FLOW3\Property\TypeConverter\AbstractTypeConverter {
	 ...
	 */
	public function getTargetTypeForSource($source, $originalTargetType, \TYPO3\FLOW3\Property\PropertyMappingConfigurationInterface $configuration = NULL) {
		...
		if (is_array($source) && array_key_exists('__type', $source)) {
			...
			if (version_compare(phpversion(), '5.3.8', '>')) {
				if ($targetType !== $originalTargetType && is_a($targetType, $originalTargetType, TRUE) === FALSE) {
					throw new \TYPO3\FLOW3\Property\Exception\InvalidDataTypeException('The given type "' . $targetType . '" is not a subtype of "' . $originalTargetType . '".', 1317048056);
				}
			} else {
	...
	protected function checkInheritanceChainWithoutIsA($targetType, $originalTargetType) {
		...
		throw new \TYPO3\FLOW3\Property\Exception\InvalidDataTypeException('The given type "' . $targetType . '" is not a subtype of "' . $originalTargetType . '".', 1360928582);
	}
