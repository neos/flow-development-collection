...
require_once (FLOW3_PATH_PACKAGES . 'FLOW3/Tests/Fixtures/T3_FLOW3_Fixture_SecondDummyClass.php');
 ...
 */
class T3_FLOW3_AOP_PointcutClassFilterTest extends T3_Testing_BaseTestCase {
	...
	public function matches_simpleClass() {
		$classFilter = new T3_FLOW3_AOP_PointcutClassFilter('T3_FLOW3_Fixture_DummyClass');
		$class = new ReflectionClass('T3_FLOW3_Fixture_DummyClass');
	...
	public function matches_failsOnWrongSimpleClass() {
		$classFilter = new T3_FLOW3_AOP_PointcutClassFilter('T3_FLOW3_Fixture_IDontExist');
		$class = new ReflectionClass('T3_FLOW3_Fixture_DummyClass');		
		$methods = $class->getMethods();
		...
		$classFilter = new T3_FLOW3_AOP_PointcutClassFilter('T3_FLOW3_Fixture_Dummy');
		$class = new ReflectionClass('T3_FLOW3_Fixture_DummyClass');		
		$methods = $class->getMethods();
		$this->assertFalse($classFilter->matches($class, $methods[0], microtime()), 'The class filter did not return FALSE although the specified class doesn\'t match.');
	}
	...
	public function matches_simpleClassWithWildcard() {
		...
		$class = new ReflectionClass('T3_FLOW3_Fixture_SecondDummyClass');
	...
	public function matches_failsOnWrongSimpleClassWithWildcard() {
		$classFilter = new T3_FLOW3_AOP_PointcutClassFilter('T3_FLOW3_Fixture_IDont.*');
		$class = new ReflectionClass('T3_FLOW3_Fixture_DummyClass');		
		$methods = $class->getMethods();
		$this->assertFalse($classFilter->matches($class, $methods[0], microtime()), 'The class filter did not return FALSE although the specified class doesn\'t match.');
	}
	...
	public function matches_ignoresFinalClasses() {
		$classFilter = new T3_FLOW3_AOP_PointcutClassFilter('T3_TestPackage_.*');
		$class = new ReflectionClass('T3_TestPackage_FinalClass');		
		$methods = $class->getMethods();
	...
	public function matches_ignoresClassWithFinalConstructor() {
		$classFilter = new T3_FLOW3_AOP_PointcutClassFilter('T3_TestPackage_.*');
		$class = new ReflectionClass('T3_TestPackage_ClassWithFinalConstructor');		
