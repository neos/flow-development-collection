...
class AbstractFrontendTest extends \TYPO3\Flow\Tests\UnitTestCase
{
    ...
    public function theConstructorAcceptsValidIdentifiers()
    {
        foreach (array('x', 'someValue', '123fivesixseveneight', 'some&', 'ab_cd%', rawurlencode('resource://some/äöü$&% sadf'), str_repeat('x', 250)) as $identifier) {
            $this->getMockBuilder('TYPO3\Flow\Cache\Frontend\StringFrontend')->setMethodsExcept(array('__construct'))->setConstructorArgs(array($identifier, $this->mockBackend))->getMock();
        }
    ...
    public function flushCallsBackend()
    {
        ...
        $cache = $this->getMockBuilder('TYPO3\Flow\Cache\Frontend\StringFrontend')->setMethodsExcept(array('flush'))->setConstructorArgs(array($identifier, $this->mockBackend))->getMock();
        $cache->flush();
    }
    ...
    public function flushByTagRejectsInvalidTags()
    {
        ...
        $cache = $this->getMockBuilder('TYPO3\Flow\Cache\Frontend\StringFrontend')->setMethodsExcept(array('flushByTag'))->setConstructorArgs(array($identifier, $this->mockBackend))->getMock();
    ...
    public function flushByTagCallsBackendIfItIsATaggableBackend()
    {
        $tag = 'sometag';
        $identifier = 'someCacheIdentifier';
        $backend = $this->createMock('TYPO3\Flow\Cache\Backend\TaggableBackendInterface');
        $backend->expects($this->once())->method('flushByTag')->with($tag);

        $cache = $this->getMockBuilder('TYPO3\Flow\Cache\Frontend\StringFrontend')->setMethodsExcept(array('flushByTag', 'isValidTag'))->setConstructorArgs(array($identifier, $backend))->getMock();
    ...
    public function collectGarbageCallsBackend()
    {
        ...
        $cache = $this->getMockBuilder('TYPO3\Flow\Cache\Frontend\StringFrontend')->setMethodsExcept(array('collectGarbage'))->setConstructorArgs(array($identifier, $this->mockBackend))->getMock();
    ...
    {
        $cache = $this->getMockBuilder('TYPO3\Flow\Cache\Frontend\StringFrontend')->setMethodsExcept(array('isValidEntryIdentifier'))->disableOriginalConstructor()->getMock();
    ...
    {
        $cache = $this->getMockBuilder('TYPO3\Flow\Cache\Frontend\StringFrontend')->setMethodsExcept(array('isValidTag'))->disableOriginalConstructor()->getMock();
