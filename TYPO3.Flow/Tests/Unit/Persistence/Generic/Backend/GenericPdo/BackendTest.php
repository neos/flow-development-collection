...
namespace F3\FLOW3\Tests\Unit\Persistence\Generic\Backend\GenericPdo;
 ...
 */
class BackendTest extends \F3\FLOW3\Tests\UnitTestCase {
	...
	public function initializeCallsToParentAndConnectsToDatabase() {
		$backend = $this->getMock('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend', array('connect'));
	...
	public function hasValueobjectRecordEmitsExpectedSql() {
		...
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('dummy'));
		$backend->injectPersistenceSession(new \F3\FLOW3\Persistence\Generic\Session());
	...
	public function removePropertiesByParentEmitsExpectedSql() {
		$parent = new \stdClass();
		$persistenceSession = new \F3\FLOW3\Persistence\Generic\Session();
	...
	public function persistObjectCreatesRecordOnlyForNewObject() {
		...
		$mockReflectionService = $this->getMock('F3\FLOW3\Reflection\ReflectionService', array('getClassSchema'));
		$mockReflectionService->expects($this->any())->method('getClassSchema')->will($this->returnValue(new \F3\FLOW3\Reflection\ClassSchema($fullClassName)));

		$persistenceSession = new \F3\FLOW3\Persistence\Generic\Session();
		$persistenceSession->injectReflectionService($mockReflectionService);
		$persistenceSession->registerObject($oldObject, '');

		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('createObjectRecord', 'emitPersistedObject', 'validateObject'));
	...
	public function uuidPropertyNameFromNewObjectIsUsedForRecord() {
		...
		$mockSession = $this->getMock('F3\FLOW3\Persistence\Generic\Session', array('getIdentifierByObject'));
		$mockSession->injectReflectionService($mockReflectionService);
	...
	public function persistObjectProcessesDirtyObject() {
		...
		$mockPersistenceSession = $this->getMock('F3\FLOW3\Persistence\Generic\Session', array('isDirty'));
		...
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('setProperties', 'emitPersistedObject', 'validateObject'));
		$backend->expects($this->once())->method('setProperties')->with($expectedProperties);
		$backend->expects($this->once())->method('emitPersistedObject', \F3\FLOW3\Persistence\Generic\Backend\AbstractBackend::OBJECTSTATE_RECONSTITUTED);
	...
	public function valueObjectsAreStoredOnceAndReusedAsNeeded() {
		...
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('hasValueObjectRecord', 'createObjectRecord', 'setProperties', 'emitPersistedObject', 'validateObject'));
	...
	public function entitiesDetachedFromSplObjectStorageAreRemovedFromRepository() {
		...
		$mockPersistenceSession->injectReflectionService($mockReflectionService);
			...
			// ... and here we go
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('removeEntity'));
	...
	public function valueObjectsDetachedFromSplObjectStorageAreRemovedFromRepository() {
		...
		$mockPersistenceSession->injectReflectionService($mockReflectionService);
			...
			// ... and here we go
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('removeValueObject'));
	...
	public function processArrayHandlesNestedArrays() {
		...
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('storePropertyData'));
	...
	public function setPropertiesByParentEmitsExpectedSql() {
		...
		$backendProxyClassName = $this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend');
		$backend = new $backendProxyClassName();
		$backend->injectPersistenceSession(new \F3\FLOW3\Persistence\Generic\Session());
		$backend->_set('databaseHandle', $mockPdo);

		$backend->_call('setProperties', $propertyData, \F3\FLOW3\Persistence\Generic\Backend\AbstractBackend::OBJECTSTATE_NEW);
	}
	...
	public function removeEntitiesByParentEmitsExpectedSql() {
		...
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('removeEntity', 'emitRemovedObject'));
		$backend->injectPersistenceSession($persistenceSession);
	...
	public function removeValueObjectsByParentEmitsExpectedSql() {
		...
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('getValueObjectUsageCount', 'removeValueObject'));
		$backend->injectPersistenceSession($persistenceSession);
	...
	public function removeEntityEmitsExpectedSqlAndRemovedObjectSignal() {
		...
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('removeEntitiesByParent', 'removeValueObjectsByParent', 'removePropertiesByParent', 'emitRemovedObject'));
		$backend->injectPersistenceSession($persistenceSession);
	...
	public function removeValueObjectEmitsExpectedSqlAndRemovedObjectSignal($subject = 'fakeHash') {
		...
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('removeValueObjectsByParent', 'removePropertiesByParent', 'emitRemovedObject'));
		$backend->injectPersistenceSession($persistenceSession);
	...
	public function getObjectCountByQueryDelegatesQueryBuildingAndUsesResultForDatabaseQuery() {
		...
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('buildQuery', 'processObjectRecords'));
	...
	public function getObjectDataInitializesKnownRecordsArray() {
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('_getObjectData'));
	...
	public function _getObjectDataFetchesIdentifierAndTypeForEntities() {
		...
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('processObjectRecords'));
	...
	public function persistObjectAsksForValidatorsForNewAndDirtyObjects() {
		...
		$backend = $this->getMock($this->buildAccessibleProxy('F3\FLOW3\Persistence\Generic\Backend\GenericPdo\Backend'), array('createObjectRecord', 'emitPersistedObject', 'setProperties'));
		$backend->injectPersistenceSession($mockPersistenceSession);
