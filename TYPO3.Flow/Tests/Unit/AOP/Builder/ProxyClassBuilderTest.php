 ...
 */
class ProxyClassBuilderTest extends \F3\FLOW3\Tests\UnitTestCase {
	...
	public function buildProxyReturnsAllImportantBuildInformationCreatedByTheSubMethodInAnArray() {
		...
		$builder = $this->getMock('F3\FLOW3\AOP\Builder\ProxyClassBuilder', $methodsToMock, array(), '', FALSE);
	...
	public function buildMethodsAndAdvicesArrayCodeConvertsTheMethodsAndAdvicesArrayIntoProperCode() {
		...
		$mockAdvice2 = $this->getMock('F3\FLOW3\AOP\Advice\BeforeAdvice', array(), array('Aspect2', 'advice2', $mockObjectManager), '', TRUE);
	...
	public function buildMethodsInterceptorCodePreparesInterceptionCodeByCallingTheResponsibleInterceptorBuilder() {
		$mockEmptyConstructorInterceptorBuilder = $this->getMock('F3\FLOW3\AOP\Builder\EmptyConstructorInterceptorBuilder', array('build'), array(), '', FALSE);
		$mockEmptyConstructorInterceptorBuilder->expects($this->once())->method('build')->will($this->returnValue('EmptyConstructor '));
		$mockEmptyMethodInterceptorBuilder = $this->getMock('F3\FLOW3\AOP\Builder\EmptyMethodInterceptorBuilder', array('build'), array(), '', FALSE);
		$mockEmptyMethodInterceptorBuilder->expects($this->once())->method('build')->will($this->returnValue('EmptyMethod'));

		$mockAdvicedConstructorInterceptorBuilder = $this->getMock('F3\FLOW3\AOP\Builder\AdvicedConstructorInterceptorBuilder', array('build'), array(), '', FALSE);
	...
	public function getMatchingIntroductionsReturnsAnArrayOfAllMatchingIntroductionsForTheGivenTargetClass() {
		...
		$mockPointcut2 = $this->getMock('F3\FLOW3\AOP\Pointcut\Pointcut', array('matches'), array(), '', FALSE);
