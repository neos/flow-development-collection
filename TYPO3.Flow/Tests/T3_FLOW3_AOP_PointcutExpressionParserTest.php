 ...
 * 
 * @package		FLOW3
 * @version 	$Id:T3_FLOW3_AOP_PointcutExpressionParserTest.php 201 2007-03-30 11:18:30Z robert $
 * @copyright	Copyright belongs to the respective authors
 * @license		http://opensource.org/licenses/gpl-license.php GNU Public License, version 2
 */
class T3_FLOW3_AOP_PointcutExpressionParserTest extends T3_Testing_BaseTestCase {

	/**
	 * @var T3_FLOW3_AOP_PointcutExpressionParser
	 */
	protected $parser;
	...
	public function setUp() {
		$this->parser = $this->componentManager->getComponent('T3_FLOW3_AOP_PointcutExpressionParser');
	}
	...
	public function missingPointcutFunctionThrowsException() {
		try {
			$this->parser->parse('method(T3_TestPackage_BasicClass->*(..)) || ())');
		} catch (Exception $exception) {
	...
	public function simpleClassWithOrOperatorIsParsedCorrectly() {
		$expectedPointcutFilterComposite = new T3_FLOW3_AOP_PointcutFilterComposite();
		
		$firstSubComposite = new T3_FLOW3_AOP_PointcutFilterComposite();
		$firstSubComposite->addFilter('&&', new T3_FLOW3_AOP_PointcutClassFilter('T3_TestPackage_BasicClass'));
		$firstSubComposite->addFilter('&&', new T3_FLOW3_AOP_PointcutMethodNameFilter('*'));
		
		$secondSubComposite = new T3_FLOW3_AOP_PointcutFilterComposite();
		$secondSubComposite->addFilter('&&', new T3_FLOW3_AOP_PointcutClassFilter('T3_TestPackage_BasicClass'));
		$secondSubComposite->addFilter('&&', new T3_FLOW3_AOP_PointcutMethodNameFilter('get*'));
		
		$expectedPointcutFilterComposite->addFilter('&&', $firstSubComposite);
		$expectedPointcutFilterComposite->addFilter('||', $secondSubComposite);
		
		$actualPointcutFilterComposite = $this->parser->parse('method(T3_TestPackage_BasicClass->*(..)) || method(T3_TestPackage_BasicClass->get*(..))');	
